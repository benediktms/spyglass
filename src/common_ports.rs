// exact location of nmap-services will depend on OS/package manager
// e.g. for macOs with homebrew:
// awk '$2~/tcp$/' /opt/homebrew/opt/nmap/share/nmap/nmap-services | sort -r -k3 | head -n 1000 | tr -s ' ' | cut -d '/' -f1 | sed 's/\S*\s*\(\S*\).*/\1,/' >> common_ports.txt
